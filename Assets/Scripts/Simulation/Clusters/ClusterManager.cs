...
namespace FallingSand
{
    ...
    public class ClusterManager : MonoBehaviour
    {
        ...
        /// <summary>
        /// Remove a cluster completely: clear its grid pixels, unregister, and destroy the GameObject.
        /// Use this for any external system that needs to destroy a cluster cleanly.
        /// </summary>
        ...
        public void StepAndSync(float deltaTime)
        {
            ...
            foreach (var cluster in clusters.Values)
            {
                if (cluster.rb != null && !cluster.rb.IsSleeping())
                {
                    ...
                    if (linVel < 3f && contactCount > 0)
                    {
                        ...
                        {
                            // Don't sleep if any force provider is acting on this cluster,
                            // or if it's a machine part (piston arm), or being crushed
                            if (cluster.activeForceCount > 0 || cluster.isMachinePart || cluster.crushPressureFrames > 0)
                            {
            ...
            ClusterFracturer.CheckCompressionAndFracture(this);
        ...
        /// <summary>
        /// Compute the cell-space bounding box for a cluster's inverse mapping,
        /// then iterate each cell that maps to a valid pixel, calling the provided action.
        /// Deduplicates the shared setup between ClearClusterPixels and SyncClusterToWorld.
        /// </summary>
        private void ForEachClusterCell(ClusterData cluster,
            System.Action<int, int, int, byte> cellAction)
        {
            ...
            for (int cy = cellMinY; cy <= cellMaxY; cy++)
            {
                for (int cx = cellMinX; cx <= cellMaxX; cx++)
                {
                    ...
                    cellAction(cx, cy, index, materialId);
                }
            }
        }
        ...
        {
            ushort clusterId = cluster.clusterId;
            ForEachClusterCell(cluster, (cx, cy, index, materialId) =>
            {
        ...
        {
            ushort clusterId = cluster.clusterId;
            Vector2 clusterVelocity = cluster.Velocity;
            ...
            ForEachClusterCell(cluster, (cx, cy, index, materialId) =>
            {
                ...
                {
                    DisplaceCell(new Vector2Int(cx, cy), existing, clusterVelocity);
                }
        ...
        private void DisplaceCell(Vector2Int fromPos, Cell cell, Vector2 clusterVelocity)
        {
            ...
            {
                newPos = emptySpot.Value;
            }
            else
            {
                // BFS exhausted — spawn above the congestion zone to preserve material
                newPos = FindEmptyCellAbove(fromPos);
            }
        ...
        /// <summary>
        /// Scan upward from a position to find the first empty cell.
        /// Used as a fallback when BFS can't find space — ensures material is never lost.
        /// </summary>
        private Vector2Int FindEmptyCellAbove(Vector2Int from)
        {
            int x = from.x;
            // Cell Y+ is down, so scan toward Y=0 (upward in world)
            ...
            // If entire column above is full, try scanning down
            ...
            // Absolute last resort: overwrite the top cell (extremely unlikely)
